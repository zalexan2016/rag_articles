...
class Pipeline:
    ...
    def run(self) -> PipelineStats:
        ...
        files_to_process = self._filter_new_files(md_files, stats)

        if not files_to_process:
            logger.info("No new files require processing. All %s files are up to date.", stats.total_found)
        ...
        for idx, (filename, content) in enumerate(files_to_process, start=1):
            ...
            self._process_file(filename, content, stats)

        self._log_summary(stats)
        return stats

    def _filter_new_files(self, md_files: list[Path], stats: PipelineStats) -> list[tuple[str, str]]:
